...
- [The Turning Way](https://the-turing-way.netlify.app/index.html) handbook to reproducible, ethical and collaborative data science.
...
There were `{r} n_attendees` attendees from `{r} n_depts_attendees` departments representing `{r} n_coll_attendees` colleges.
...
```{r fig-attendees-by-position, fig.cap="Registrations by position", eval=update_figs}
attendees_clean |>
  ggplot() +
  aes(x = position, fill = position) +
  geom_bar() +
